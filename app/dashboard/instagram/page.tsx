...
import { getInstagramUiContract, getInstagramExternalUiContract } from '@/lib/queries/instagram-ui';
...
export default async function InstagramPage({ searchParams }: PageProps) {
  ...
  const [contract, externalContract] = await Promise.all([
    getInstagramUiContract({
    ...
    getInstagramExternalUiContract({
      candidateIds: candidateIds ?? undefined,
      periodDays: globalFilters.period === 'all' ? null : Number(globalFilters.period),
      contentTypes: format && ['IMAGE', 'REEL', 'CAROUSEL'].includes(format) ? [format as 'IMAGE' | 'REEL' | 'CAROUSEL'] : undefined,
      risk: first(params.risk) ?? null,
      sentiment: first(params.sentiment) ?? null,
      topic: first(params.topic) ?? null,
      origin: first(params.origin) ?? null,
      page,
      pageSize: 20,
    }),
  ]);

  const activeTab = (first(params.tab) === 'external' ? 'external' : 'owned') as 'owned' | 'external';
  const filterOptions = activeTab === 'external'
    ? {
        ...externalContract.filterOptions,
        candidates: contract.filterOptions.candidates,
      }
    : contract.filterOptions;
  ...
  return (
    <main className="space-y-6">
      <header className="flex items-center gap-3 border-b border-white/[0.08] pb-3 mb-1">
        ...
        <span className="text-[10px] text-slate-500 leading-none">Desempenho editorial, pressão social e menções externas.</span>
      ...
      <Suspense fallback={<div className="h-16 animate-pulse rounded-md bg-white/5" />}>
        <InstagramUiFilters options={filterOptions} activeTab={activeTab} />
      </Suspense>
