...
export function PesquisasExplicamCenario({ polls, latestPoll, onNavigateToLista }: Props) {
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between pb-2 border-b border-white/5 flex-wrap gap-2">
          <div>
            <h3 className="text-white font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 truncate">
              <FileText size={14} className="text-blue-500 shrink-0" /> Pesquisas Explicam o Cenário ({topPolls.length})
            </h3>
          </div>
        ...
        <div className="space-y-2 pt-1">
          {topPolls.map((poll) => {
            ...
            const primaryCenario = t1[0]?.cenario ?? null;
            ...
            const compExplanation = explainComparabilityReason(
              latestPoll,
              poll,
              latestPollResults,
              poll.results
