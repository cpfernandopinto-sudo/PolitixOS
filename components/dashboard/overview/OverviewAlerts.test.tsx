...
describe('OverviewAlerts — resumo operacional (reintegração Sprint 6)', () => {
  ...
  it('limita a exibição inicial a no máximo 3 alertas (hotfix de apresentação)', () => {
  ...
  it('não mostra "Ver todos" — a Central de Alertas não foi transportada nesta release (integração Radar real)', () => {
    const many = Array.from({ length: 10 }, (_, i) => makeRisk({ id: `r${i}`, descricao: `Risco ${i}` }));
    render(<OverviewAlerts risks={many} />);
    expect(screen.queryByRole('link', { name: 'Ver todos' })).not.toBeInTheDocument();
    expect(screen.queryByText(/\/dashboard\/alertas/)).not.toBeInTheDocument();
  });
});
