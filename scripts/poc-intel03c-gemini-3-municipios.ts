/**
 * INTEL-03C — POC real com Gemini: Contagem + Betim + Belo Horizonte (Parte D do gate).
 *
 * Mesma seleção EXATA do INTEL-03B (`scripts/poc-intel03b-interpretation-llm-3-municipios.ts`)
 * — mesmos municípios, mesmas fontes, mesmo `EconomicIntelligenceResult` — trocando o
 * provider para `GeminiInterpretationProvider` via `createInterpretationProvider`
 * (config única, INTEL_LLM_PROVIDER=gemini) e o prompt para V2 por padrão. O baseline
 * Anthropic NÃO é re-executado aqui (seção 35-36 do gate — 14 chamadas já estão
 * congeladas no relatório do INTEL-03B); este script produz apenas o lado Gemini do
 * benchmark. Não persiste nada.
 *
 * Gated: só executa a chamada real se `RUN_REAL_INTEL_LLM=1` E `GEMINI_API_KEY`
 * estiverem presentes. Sem isso, imprime BLOCKED_BY_CREDENTIAL e encerra sem qualquer
 * chamada de rede ao provider — nunca inventa resultado.
 */
...
import { createInterpretationProvider } from '../lib/territorios/intelligence/interpretation/config';
...
async function main() {
  ...
  const hasCredential = Boolean(process.env.GEMINI_API_KEY);

  if (!runRealFlag || !hasCredential) {
    console.log('=== INTEL-03C POC real (Gemini) — BLOCKED_BY_CREDENTIAL ===');
    console.log(`RUN_REAL_INTEL_LLM=${process.env.RUN_REAL_INTEL_LLM ?? '(ausente)'} (esperado "1")`);
    console.log(`GEMINI_API_KEY presente: ${hasCredential}`);
    console.log('Nenhuma chamada de rede foi feita. Nenhum resultado foi inventado. Para executar de verdade:');
    console.log('  RUN_REAL_INTEL_LLM=1 GEMINI_API_KEY=... INTEL_LLM_PROVIDER=gemini npx tsx scripts/poc-intel03c-gemini-3-municipios.ts');
    return;
  }
  ...
  const provider = createInterpretationProvider({ providerId: 'gemini', model: process.env.INTEL_LLM_MODEL ?? 'gemini-2.5-flash', promptVersion: (process.env.INTEL_LLM_PROMPT as 'v1' | 'v2' | 'v3' | undefined) ?? 'v2' });
  ...
  for (const { codigoIbge, nome } of MUNICIPALITIES) {
    ...
    totalAccepted += pipeline.accepted.length;
    totalRejected += pipeline.rejected.length;
    totalCalls += pipeline.executionMetadata.length;
    if (pipeline.rejected.length > 0) {
      console.log('REJEITADAS:');
      for (const rejected of pipeline.rejected) {
        console.log('  ', rejected.draft.id, JSON.stringify(rejected.errors));
        for (const error of rejected.errors) errorCodeCounts[error.code] = (errorCodeCounts[error.code] ?? 0) + 1;
      }
    }
    ...
    if (secondRun.status === 'COMPLETED') {
      ...
      console.log(`  consistencyScore: ${JSON.stringify({
        sameCount: pipeline.accepted.length === secondRun.accepted.length,
        sameRefSets: setsEqual(refSet(pipeline.accepted), refSet(secondRun.accepted)),
        sameClaimTypeSets: setsEqual(claimTypeSet(pipeline.accepted), claimTypeSet(secondRun.accepted)),
      })}`);
    } else {
  ...
  console.log(`Chamadas de produção: ${totalCalls} | aceitas: ${totalAccepted} | rejeitadas: ${totalRejected} | taxa de aceitação: ${totalCalls > 0 ? ((totalAccepted / (totalAccepted + totalRejected)) * 100).toFixed(1) : '0'}%`);
  console.log(`Códigos de erro nas rejeições: ${JSON.stringify(errorCodeCounts)}`);
